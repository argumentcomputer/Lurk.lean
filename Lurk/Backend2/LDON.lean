...
-- class MonadLDONHash (m : Type → Type) where
--   getHashState : m LDONHashState
--   modifyHashState : (LDONHashState → LDONHashState) → m Unit

-- export MonadLDONHash (getHashState modifyHashState)

-- @[always_inline]
-- instance (m n) [MonadLift m n] [MonadLDONHash m] : MonadLDONHash n where
--   getHashState    := liftM (getHashState : m LDONHashState)
--   modifyHashState := fun f => liftM (modifyHashState f : m Unit)

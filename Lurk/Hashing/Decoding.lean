  ...
  | none =>
    if (← read).visit.contains ptr then throw "Cycle of pointers detected"
      ...
      let ast ← match ptr with
        ...
        | ⟨.strNil, F.zero⟩ => return .str default
        | ptr => match (← read).store.exprs.find? ptr with
          ...
          | some expr => match (ptr.tag, expr) with
            ...
            | (.str, .str s) => match (s.tag, ← decodeAST s) with
              | (.strCons, .str s) => return .str s
              | _ => throw s!"Error when decoding a str pointer: {s}"
            | (.strCons, .strCons h t) => match (h.tag, ← decodeAST t) with
              | (.char, .str t) => return .str ⟨Char.ofNat h.val :: t.data⟩
              | _ => throw s!"Error when decoding strCons pointers: {h}; {t}"
